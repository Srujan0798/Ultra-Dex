...
export function registerCiMonitorCommand(program) {
  program
    ...
    .option(
      '--notify-on <events>',
      'Comma-separated events: failure,success,fix (default: failure)',
      'failure'
    )
            ...
            try {
              ...
              if (event === 'workflow_job' && payload.action === 'completed') {
                if (payload.workflow_job.conclusion === 'failure') {
                  ...
                  res.end(
                    JSON.stringify({ status: 'accepted', message: 'Processing build failure...' })
                  );
...
async function handleBuildFailure(payload, options, notifyEvents) {
  ...
  if (options.heal) {
    await runAgentLoop(
      'devops',
      `Commit the fix to a new branch 'fix/${job.name.toLowerCase().replace(/[^a-z0-9]/g, '-')}' and push.`,
      provider,
      context
    );
  }
  ...
  try {
    ...
    if (type === 'failure') {
      discordPayload = {
        ...
        avatar_url:
          'https://raw.githubusercontent.com/Srujan0798/Ultra-Dex/main/cli/assets/logo.png',
        embeds: [
          {
            title: '🚨 Build Failed',
            color: 0xff0000, // Red
