...
HashEmbeddingModel = __decorateClass([
  singleton()
], HashEmbeddingModel);
...
TransformersEmbeddingModel = __decorateClass([
  singleton()
], TransformersEmbeddingModel);
let SemanticRouter = class {
  constructor(options = {}) {
    this.dimensions = options.dimensions || DEFAULT_DIMENSIONS;
    this.backend = options.backend || process.env.ULTRA_DEX_ROUTER_BACKEND || (process.env.NODE_ENV === "test" ? "hashed" : "hashed");
    this.modelName = options.modelName || DEFAULT_MODEL;
    this.embedder = options.embedder || (this.backend === "transformers" ? new TransformersEmbeddingModel({
  ...
  retrainSync(profiles = AGENT_PROFILES) {
    this.agentProfiles = normalizeProfiles(profiles);
    this.profileEmbeddings = this.agentProfiles.map(
      (profile) => this.embedder.embedSync(profile.searchableText)
    );
    return this.agentProfiles;
  }
  ensureReadySync() {
  ...
  buildMatches(taskEmbedding) {
    return this.agentProfiles.map((profile, index) => {
...
SemanticRouter = __decorateClass([
  singleton()
], SemanticRouter);
let HybridRouter = class {
  ...
  capabilityScore(taskText, profile, requiredCapabilities = []) {
    ...
    const lexicalMatches = Array.from(profile.capabilityTokenSet).filter(
      (token) => taskTokens.has(token)
    ).length;
    const lexicalScore = profile.capabilityTokenSet.size > 0 ? lexicalMatches / Math.max(1, Math.min(profile.capabilityTokenSet.size, taskTokens.size || 1)) : 0;
    if (!requiredCapabilities.length) {
      return Number(clamp(lexicalScore, 0, 1).toFixed(4));
    }
    const matchedRequired = requiredCapabilities.filter(
  ...
  rankMatches(taskText, semanticMatches, requiredCapabilities = []) {
    return semanticMatches.map((match) => {
      const feedbackAdjustment = typeof this.semanticRouter.getFeedbackAdjustment === "function" ? this.semanticRouter.getFeedbackAdjustment(match.agentId) : 1;
      const adjustedSemanticConfidence = clamp(match.confidence * feedbackAdjustment, 0, 0.99);
      const capabilityScore = this.capabilityScore(
        taskText,
        match.profile,
        requiredCapabilities
      );
      const hybridScore = adjustedSemanticConfidence * this.semanticWeight + capabilityScore * this.capabilityWeight;
      return {
        ...match,
        capabilityScore,
  ...
  routeSync(task, requiredCapabilities = []) {
    ...
    if (ranked[0].confidence < this.minimumSemanticConfidence) {
      const capabilityRanked = requiredCapabilities.length ? [...ranked].sort(
        (left, right) => right.capabilityScore - left.capabilityScore || right.hybridScore - left.hybridScore
      ) : ranked;
