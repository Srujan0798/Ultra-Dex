...
let HybridRouter = class {
  ...
  async route(task, requiredCapabilities) {
    ...
    try {
      ...
      const capableAgents = this.agentRegistry.findAgentsByCapabilities(
        requiredCapabilities
      );
  ...
  async routeBatch(tasks) {
    return Promise.all(
      tasks.map((t) => this.route(t.task, t.requiredCapabilities))
    );
  }
  /**
  ...
  calculateCapabilityScore(agentCapabilities, requiredCapabilities) {
    ...
    const matches = requiredCapabilities.filter(
      (req) => normalizedAgentCaps.includes(req.toLowerCase())
    );
