...
class CommandHistory {
  ...
  async save() {
    ...
    try {
      ...
      logger.error("Failed to save history:", error instanceof Error ? error.message : String(error));
    }
  }
  ...
  async search(query) {
    ...
    return this.history.filter(
      (h) => h.command.toLowerCase().includes(lowerQuery) || h.args.some((a) => a.toLowerCase().includes(lowerQuery)) || h.cwd.toLowerCase().includes(lowerQuery)
    ).reverse();
  }
  async clear() {
    this.history = [];
...
async function registerHistoryCommand(program) {
  ...
  const listCmd = historyCmd.command("list").alias("ls").description("Show recent commands").option("-n, --limit <num>", "Number of entries", "20").option("-c, --command <cmd>", "Filter by command").option("--success", "Show only successful").option("--failed", "Show only failed");
  listCmd.action(
    async (options) => {
      const filter = {};
      if (options.command)
        filter.command = options.command;
      if (options.success)
      ...
      entries.forEach((e) => {
        ...
        const result = e.result === "success" ? chalk.green("\u2713") : e.result === "error" ? chalk.red("\u2717") : chalk.yellow("\u25CB");
...
Total: ${entries.length} entries`));
  ...
  const replayCmd = historyCmd.command("replay <id>").description("Replay a command from history").option("--dry-run", "Show what would be executed");
  replayCmd.action(
    async (id, options) => {
      try {
        await history.replay(id, options.dryRun);
      } catch (error) {
        logger.error(chalk.red("Replay failed:"), error instanceof Error ? error.message : String(error));
      }
    }
  );
...
`));
  ...
  statsCmd.action(
    async () => {
      ...
      if (Object.keys(stats.byCommand).length > 0) {
        ...
        const sorted = Object.entries(stats.byCommand).sort((a, b) => b[1] - a[1]).slice(0, 10);
  ...
  const clearCmd = historyCmd.command("clear").description("Clear command history").option("--force", "Skip confirmation");
  clearCmd.action(
    async (options) => {
      if (!options.force) {
