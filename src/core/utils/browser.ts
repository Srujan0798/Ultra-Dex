...
let BrowserAutomation = class {
  ...
  async launch() {
    try {
      ...
      logger.log(chalk.yellow(`Browser launch failed: ${err instanceof Error ? err.message : String(err)}`));
  ...
  async getPageContent(url) {
    ...
    const headings = await this.page.evaluate(() => {
      return Array.from(document.querySelectorAll("h1, h2, h3")).slice(0, 20).map((h) => ({
  ...
  async screenshot(url, options = {}) {
    ...
    const filename = customPath || path.join(this.options.screenshotDir, `screenshot-${timestamp}.png`);
...
BrowserAutomation = __decorateClass([
  singleton()
], BrowserAutomation);
async function simpleFetch(url) {
  ...
  if (contentType.includes("text/html")) {
    const cleaned = text.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, "").replace(/<style\b[^<]*(?:(?!<\/style>)<[^<]*)*<\/style>/gi, "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
