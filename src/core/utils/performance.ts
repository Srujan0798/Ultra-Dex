...
function withPerformance(commandName, fn) {
  return async function(...args) {
    ...
    try {
      ...
      const metric = tracker.end("error", error instanceof Error ? error : new Error(String(error)));
      tracker.save();
...
function registerPerformanceCommand(program) {
  program.command("perf").alias("performance").description("Performance monitoring and analytics").option("--summary", "Show performance summary").option("--days <n>", "Number of days for summary", "7").option("--operation <name>", "Filter by operation name").option("--export <file>", "Export metrics to file").option("--clear", "Clear performance history").option("--compare", "Compare current vs previous runs").action(async (options) => {
    ...
    if (options.export) {
      ...
      const exportData = options.operation ? data.filter((d) => d.operation === options.operation) : data;
      fs.writeFileSync(options.export, JSON.stringify(exportData, null, 2));
      logger.log(chalk.green(`\u2705 Exported ${exportData.length} metrics to ${options.export}`));
      return;
    }
    if (options.summary || !options.operation) {
      ...
      logger.log(chalk.white.bold(`\u{1F4CA} Summary (Last ${summary.period})
`));
      logger.log(chalk.gray(`Total Operations: ${summary.totalOperations}
`));
      const ops = Object.entries(summary.operations).sort(
        (a, b) => b[1].count - a[1].count
      );
      ops.forEach(([name, stats]) => {
        const color = stats.successRate >= 90 ? "green" : stats.successRate >= 70 ? "yellow" : "red";
    ...
    if (options.operation) {
      ...
      logger.log(chalk.white.bold(`\u{1F4C8} Recent runs of: ${options.operation}
`));
