...
let BudgetManager = class {
  ...
  setBudget(scope, id, limit) {
    if (scope === "agent")
      this.agentBudgets.set(id, limit);
    else if (scope === "session")
      this.sessionBudgets.set(id, limit);
  ...
  getSpending(scope = "global", id = null) {
    if (scope === "global")
      return { spent: this.spent.global, budget: this.globalBudget, utilization: this.spent.global / this.globalBudget };
    if (scope === "agent" && id)
      return { spent: this.agentSpent.get(id) || 0, budget: this.agentBudgets.get(id) || Infinity };
    if (scope === "session" && id)
      return { spent: this.sessionSpent.get(id) || 0, budget: this.sessionBudgets.get(id) || Infinity };
    return null;
  }
  getAllSpending() {
    const agents = {};
...
let CostTracker = class {
  ...
  forecast(windowMs = 36e5) {
    ...
    if (recent.length === 0)
      return { burnRatePerHour: 0, projectedDailyCost: 0 };
...
export {
  ...
  ResponseCache,
  TokenGuard,
  token_guard_default as default
};
