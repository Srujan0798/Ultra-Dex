...
export function createSessionScope(sessionId, overrides = []) {
  return createSessionContainer(sessionId, overrides);
}

export function getSessionId(targetContainer) {
  return resolveOptional(DI_TOKENS.sessionId, targetContainer);
}

export function resolveSessionOrchestrator(sessionId, overrides = []) {
