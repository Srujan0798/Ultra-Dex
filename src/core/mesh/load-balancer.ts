...
let LoadBalancer = class {
  ...
  rebalance() {
    ...
    for (const overWorker of overloaded) {
      const tasksToMove = Math.ceil(
        (overWorker.load - 0.5) * overWorker.capabilities.maxConcurrentTasks
      );
  ...
  capabilityScore(worker, requiredCapabilities) {
    let score = 50;
    const allCapabilities = [...worker.capabilities.agentTypes, ...worker.capabilities.skills];
  ...
  updateStats(decisionTime, strategy) {
    this.stats.averageDecisionTime =
      (this.stats.averageDecisionTime * this.stats.totalRoutings + decisionTime) /
      (this.stats.totalRoutings + 1);
