 ...
 * Manages multi-instance agent coordination with load balancing, failover, and inter-instance communication
 */
export class DistributedCoordinator extends EventEmitter {
  constructor(options = {}) {
    ...
    this.peers = new Map(); // peerId -> { id, url, status, lastSeen, capabilities, load }
    this.taskQueue = [];
    ...
    this.wsServer = null;
    this.wsClients = new Map(); // peerId -> WebSocket connection
   ...
   */
  setupHealthChecks() {
    // Register peer connectivity check
  ...
  async handleTaskDelegation(req, res) {
    const { task, priority = 1, timeout = 30000 } = req.body;
    ...
    // Try to delegate to peer with lowest load
  ...
  async performHealthChecks() {
    const now = Date.now();
    const timeoutThreshold = this.options.heartbeatInterval * 3; // 3x heartbeat interval
  ...
  startDiscovery() {
    ...
    }, this.options.heartbeatInterval * 2); // Every 2 heartbeats
  }
  ...
  async discoverPeers() {
    ...
    for (const discoveryUrl of this.options.discoveryUrls) {
      try {
        const response = await axios.get(`${discoveryUrl}/api/v1/peers`, { timeout: 5000 });
  ...
  async connectToPeer(peerInfo) {
    try {
      if (peerInfo.url.startsWith('ws://') || peerInfo.url.startsWith('wss://')) {
        ...
        // For HTTP-only peers, just register them
  ...
  async handlePeerFailure(peerId) {
    ...
    // Redistribute tasks from failed peer
  ...
  async processTaskQueue() {
    ...
    if (this.options.enableLoadBalancing && this.shouldDelegateTask()) {
      ...
      if (targetPeer) {
        ...
        this.taskQueue.unshift(task); // Put back if no peer available
      }
    }
  }
  ...
  selectPeerForTask(task) {
    const availablePeers = Array.from(this.peers.values())
      ...
      .sort((a, b) => a.load - b.load); // Sort by load (lowest first)

    if (availablePeers.length === 0) {
      return null;
    }

    // For now, return peer with lowest load
    // In future, could consider capabilities matching
  ...
  async delegateTaskToPeer(peerId, taskData) {
    ...
    if (peer.url.startsWith('ws://') || peer.url.startsWith('wss://')) {
      ...
      // Send via HTTP
      const axios = (await import('axios')).default;
      await axios.post(`${peer.url}/api/v1/task`, taskData, { timeout: 5000 });
    } else {
  ...
  async submitTask(task, options = {}) {
    ...
    if (this.activeTasks.size < this.options.maxConcurrentTasks) {
      ...
      try {
        // Execute task using local orchestrator/execution engine
    ...
    // Queue locally if no peers available
    this.taskQueue.push({
      id: taskId,
      task,
