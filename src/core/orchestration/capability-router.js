 ...
 * Enforces strict capability matching: NO MATCH = NO EXECUTION
 */
export class CapabilityRouter {
  constructor(agentRegistry, options = {}) {
    ...
    // Cache for routing decisions: key -> { agent, timestamp }
    this.routeCache = new Map();

    // Default capability mappings for agent types
  ...
  async route({ task, required }, options = {}) {
    ...
    // Select the best matching agent (first one for now, could be enhanced with scoring)
  ...
  selectBestAgent(agents, requiredCapabilities) {
    // For now, prefer agents with more matching capabilities
    // Could be enhanced to consider agent load, performance metrics, etc.
