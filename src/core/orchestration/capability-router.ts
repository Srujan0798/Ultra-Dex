...
let CapabilityRouter = class {
  constructor(agentRegistry, options = {}) {
    ...
    this.logger = this.options.enableLogging ? logger.child({ component: "CapabilityRouter" }) : null;
  }
  /**
  ...
  async route({ task, required }, options = {}) {
    ...
    const correlationId = options.correlationId || `route_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    if (this.logger) {
    ...
    if (matchingAgents.length === 0) {
      ...
      if (this.logger) {
        this.logger.error("No matching agent found", {
          task: task.substring(0, 100),
          required,
          availableAgents: this.registry.getAllAgents().map((a) => ({ id: a.id, capabilities: a.capabilities })),
          correlationId
        });
      }
      throw error;
...
CapabilityRouter = __decorateClass([
  singleton()
], CapabilityRouter);
var capability_router_default = CapabilityRouter;
export {
  CapabilityRouter,
  capability_router_default as default
};
