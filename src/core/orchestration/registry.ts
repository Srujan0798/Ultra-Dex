...
import { registerAlias, registerSingleton } from '../di/container.js';
...
let AgentRegistry = class {
  constructor(options = {}) {
    this.options = {
      ...
      agentsPath: options.agentsPath || path.join(process.cwd(), 'agents'),
      ...options,
    };
  ...
  findAgentsByCapabilities(capabilities) {
    ...
    for (let i = 1; i < capabilities.length; i++) {
      const capabilityIds =
        this.agentCapabilities.get(capabilities[i]) || /* @__PURE__ */ new Set();
  ...
  async discoverAgents() {
    ...
    try {
      ...
      process.stdout
        .write(`\u{1F4CB} Agent Registry: Discovered ${this.agents.size} agents from index
`);
    } catch (error) {
...
AgentRegistry = __decorateClass([singleton()], AgentRegistry);
