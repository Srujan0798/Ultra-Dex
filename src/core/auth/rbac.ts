...
let RBAC = class {
  ...
  checkAccess(userId, resource, action) {
    ...
    return this.hasPermission(userId, permission) || this.hasPermission(userId, action) || this.hasPermission(userId, `${resource}:*`);
  }
  middleware() {
    return (req, res, next) => {
  ...
  getStats() {
    return {
      ...
      totalPermissions: Array.from(this.roles.values()).reduce((sum, role) => sum + role.permissions.length, 0)
    };
  }
};
RBAC = __decorateClass([
  singleton()
], RBAC);
var rbac_default = RBAC;
export {
  RBAC,
  rbac_default as default
};
