...
/**
 * Observability Trace Collector — Visual Agent Debugging
 *
 * You can't debug AI agents with traditional logs. This module captures
 * the full execution graph: every thought, tool call, and decision —
 * with timeline visualization and replay capability.
 *
 * Features:
 *   1. Span-based tracing  — OpenTelemetry-inspired nested spans
 *   2. Timeline generation — waterfall view of agent execution
 *   3. Token attribution   — cost per span/step
 *   4. Error propagation   — trace which span triggered failure cascade
 *   5. Search & filter     — find traces by agent, status, time range
 *   6. Dashboard aggregate — realtime stats for the observability panel
 *
...
// ───────────────────────────────────────────────────────────────────────────
// Trace — A complete execution tree
// ───────────────────────────────────────────────────────────────────────────
...
export class Trace {
    constructor({ traceId, agentId, task = '', metadata = {} }) {
        ...
        this.endTime = null;
        this.durationMs = null;
        this.spans = new Map();     // spanId → Span
    ...
    getTimeline() {
        ...
        return {
            ...
            waterfall: spans.map((s) => ({
                ...
                cost: Math.round(s.cost * 1000000) / 1000000,
...
// ───────────────────────────────────────────────────────────────────────────
// Trace Collector — The main orchestrator
// ───────────────────────────────────────────────────────────────────────────
...
export class TraceCollector extends EventEmitter {
     ...
     */
    getDashboard() {
        const recent = [...this.traces.values()]
            .filter((t) => Date.now() - t.startTime < 3600000);
        ...
        return {
            ...
            recent: {
                ...
                totalCost: Math.round(recent.reduce((sum, t) => sum + t.totalCost, 0) * 1000000) / 1000000,
            },
