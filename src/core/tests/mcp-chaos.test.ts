...
describe('BaseMCPServer', () => {
  ...
  it('should return error for unknown tool', async () => {
    const server = new BaseMCPServer({ name: 'test' });
    await assert.rejects(() => server.callTool('nonexistent'), /not found/);
  });
...
describe('FilesystemMCPServer', () => {
  ...
  it('should read a file', async () => {
    ...
    assert.equal(result.success, true);
    assert.ok(
      result.result.content.includes('Ultra-Dex') ||
        result.result.content.includes('ultra-dex') ||
...
describe('ChaosEngine', () => {
  ...
  it('should run a single attack', async () => {
    const engine = new ChaosEngine();
    const result = await engine.runAttack('error-injection', async () => 'ok', {
      errorRate: 0.3,
      attempts: 10,
    });
  ...
  it('should run latency injection', async () => {
    const engine = new ChaosEngine();
    const result = await engine.runAttack('latency-injection', async () => 'fast', {
      minMs: 10,
      maxMs: 50,
      timeoutMs: 5000,
    });
  ...
  it('should run a full campaign', async () => {
    const engine = new ChaosEngine();
    const campaign = await engine.runCampaign(async () => 'ok', {
      name: 'test-campaign',
