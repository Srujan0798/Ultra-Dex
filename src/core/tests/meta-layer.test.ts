...
describe('CostTracker', () => {
    it('should track costs per agent and model', () => {
        ...
        ct.record({ agentId: 'a1', model: 'gpt-4o', promptTokens: 100, completionTokens: 50, costPerToken: 0.00001 });
        ct.record({ agentId: 'a2', model: 'claude', promptTokens: 200, completionTokens: 100, costPerToken: 0.000005 });
...
describe('TokenGuard', () => {
    ...
    it('should block when budget exceeded', async () => {
        ...
        await assert.rejects(
            () => guard.guard(
                { agentId: 'a1', model: 'gpt-4o', messages: msgs },
                async () => ({ content: '...' })
            ),
            /TokenGuard/
        );
...
describe('MessageBus', () => {
    it('should deliver messages to subscribers', async () => {
        ...
        let received = null;
        bus.subscribe('a1', 'updates', async (payload) => { received = payload; });
...
describe('AgentMesh', () => {
    it('should register agents and claim tasks', () => {
        ...
        mesh.submitTask({ id: 't1', description: 'Write code', priority: 5, requiredCapability: 'code' });
