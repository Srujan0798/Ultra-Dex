...
let PredictiveEngine = class {
  constructor(options = {}) {
    this.memory = options.memory || null;
    this.cache = options.cache instanceof ContextCache ? options.cache : options.cache || contextCache;
  ...
  analyzeTaskIntent(task) {
    ...
    const words = normalizedTask.toLowerCase().split(/[^a-z0-9_-]+/i).map((word) => word.trim()).filter((word) => word.length > 2);
    ...
    if (keywords.some((word) => ["query", "queries", "schema", "database", "sql", "prisma"].includes(word))) {
      intent = "data";
    } else if (keywords.some((word) => ["button", "layout", "component", "frontend", "react"].includes(word))) {
      intent = "ui";
    } else if (keywords.some((word) => ["deploy", "pipeline", "build", "docker", "ci"].includes(word))) {
      intent = "operations";
    }
    return {
      keywords,
  ...
  getStats() {
    ...
    return {
      ...this.stats,
      avgPrefetchTimeMs: completed === 0 ? 0 : Math.round(this.stats.totalPrefetchTimeMs / completed),
...
PredictiveEngine = __decorateClass([
  singleton()
], PredictiveEngine);
const predictiveEngine = new PredictiveEngine();
var predictive_engine_default = PredictiveEngine;
export {
  PredictiveEngine,
  predictive_engine_default as default,
  predictiveEngine
};
