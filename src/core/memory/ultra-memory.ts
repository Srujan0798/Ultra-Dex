  ...
  async search(query, limit = 5, options = {}) {
    ...
    const exactMatches = this.memory.filter(
      (entry) =>
        ...
        (entry.tags && entry.tags.some((tag) => tag.toLowerCase().includes(normalizedQuery)))
    );
    const fuzzyMatches = this.memory.filter((entry) => {
      if (exactMatches.includes(entry)) return false;
...
UltraMemory = __decorateClass([singleton()], UltraMemory);
const ultraMemory = new UltraMemory();
var ultra_memory_default = ultraMemory;
export { UltraMemory, ultra_memory_default as default, ultraMemory };
