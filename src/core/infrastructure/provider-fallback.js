...
export class ProviderFallback extends EventEmitter {
  ...
  addProvider(name, config = {}) {
    ...
    const provider = {
      name,
      priority:
        config.priority !== undefined ? config.priority : current?.priority !== undefined ? current.priority : 1,
      costPer1kTokens:
        config.costPer1kTokens !== undefined
          ? config.costPer1kTokens
          : current?.costPer1kTokens !== undefined
            ? current.costPer1kTokens
            : 0,
      enabled:
        config.enabled !== undefined ? config.enabled : current?.enabled !== undefined ? current.enabled : true,
