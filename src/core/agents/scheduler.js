...
// Scheduler - Task scheduling and timing control

import { EventEmitter } from 'events';

/**
 * Scheduler
 * Manages task scheduling with support for cron jobs, one-time schedules, and delays
 */
export class Scheduler extends EventEmitter {
  ...
  parseSimpleCron(cronExpression, now) {
    ...
    // Example: "0 12 * * 1" = noon on Mondays
    ...
    // Simple implementation: try next 365 days
    for (let i = 0; i < 365; i++) {
      next.setDate(next.getDate() + 1);
      next.setHours(hour === '*' ? 0 : parseInt(hour), 0, 0, 0);

      if (this.matchesCronPart(minute, next.getMinutes()) &&
  ...
  async executeScheduledTask(taskId) {
    ...
    try {
      ...
      if (schedule.retryCount < (schedule.maxRetries || 0)) {
        const retryDelay = (schedule.retryDelay || 1000) * Math.pow(2, schedule.retryCount - 1);
