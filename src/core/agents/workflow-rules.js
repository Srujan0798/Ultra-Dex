...
// Workflow Rules - Agent workflow rule definitions

import { EventEmitter } from 'events';

/**
 * WorkflowRules
 * Defines and manages workflow execution rules
 */
export class WorkflowRules extends EventEmitter {
  constructor(options = {}) {
    super();
    this.rules = new Map();
  ...
  createRuleFromTemplate(ruleId, template, parameters = {}) {
    const rule = {
      condition: typeof template.condition === 'function'
        ? template.condition
        : (ctx) => this.evaluateCondition(template.condition, ctx),
      action: typeof template.action === 'function'
        ? template.action
