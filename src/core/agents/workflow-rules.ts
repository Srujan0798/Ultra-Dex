...
let WorkflowRules = class extends EventEmitter {
  ...
  async evaluateRules(context) {
    const matchedRules = [];
    const sortedRules = Array.from(this.rules.values()).filter((r) => r.active).sort((a, b) => b.priority - a.priority);
  ...
  async executeAction(action, context) {
    ...
    if (action.type === "parallel") {
      return await Promise.all(
        action.actions.map((a) => this.executeAction(a, context))
      );
    }
  ...
  createRuleFromTemplate(ruleId, template, parameters = {}) {
    const rule = {
      condition: typeof template.condition === "function" ? template.condition : (ctx) => this.evaluateCondition(template.condition, ctx),
      action: typeof template.action === "function" ? template.action : (ctx) => this.executeAction(template.action, ctx),
...
WorkflowRules = __decorateClass([
  singleton()
], WorkflowRules);
var workflow_rules_default = WorkflowRules;
export {
  WorkflowRules,
  workflow_rules_default as default
};
