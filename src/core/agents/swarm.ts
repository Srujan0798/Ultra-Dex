...
let Swarm = class extends BaseAgent {
  ...
  async executeConsensus(task) {
    const workers = Array.from(this.members.values()).filter((m) => m.status === "active" && m.role === "worker").map((m) => m.agent);
    if (workers.length === 0) {
  ...
  async executeHierarchical(task) {
    const leaders = Array.from(this.members.values()).filter((m) => m.role === "leader" && m.status === "active");
    if (leaders.length === 0) {
  ...
  async executeTournament(task) {
    const workers = Array.from(this.members.values()).filter((m) => m.status === "active" && m.role === "worker");
    if (workers.length === 0) {
    ...
    })).filter((r) => r.success).sort((a, b) => b.score - a.score);
    if (scored.length === 0) {
...
Swarm = __decorateClass([
  singleton()
], Swarm);
var swarm_default = Swarm;
export {
  Swarm,
  swarm_default as default
};
