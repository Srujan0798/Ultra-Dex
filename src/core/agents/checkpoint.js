...
// Checkpoint - Agent state checkpointing and recovery
...
/**
 * Checkpoint
 * Manages agent state persistence for recovery and distributed systems
 */
export class Checkpoint extends EventEmitter {
  ...
  async createCheckpoint(agentId, state, metadata = {}) {
    ...
    // Enforce maximum checkpoints per agent
  ...
  async restoreFromCheckpoint(agentId, checkpointId = null) {
    ...
    // Get latest checkpoint if ID not specified
  ...
  async shutdown() {
    ...
    // Save all remaining checkpoints
