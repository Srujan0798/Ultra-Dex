...
class FinancialModel {
  constructor(options = {}) {
    this.options = {
      initialMRR: options.initialMRR || 5e3,
      // $5K MRR as per requirements
      initialUsers: options.initialUsers || 500,
      // 500 users as per requirements
      growthRate: options.growthRate || 0.15,
      // 15% monthly growth
      churnRate: options.churnRate || 0.05,
      // 5% monthly churn
      customerAcquisitionCost: options.customerAcquisitionCost || 100,
      grossMargin: options.grossMargin || 0.85,
      // 85% gross margin
      monthsToProject: options.monthsToProject || 36,
      // 3 years
  ...
  generateProjections() {
    ...
    let totalCACSpent = 0;
    let cashBalance = this.options.seedFunding;
  ...
  getAverageRevenuePerUser(month) {
    const baseArpu = 10;
    const growthFactor = 1 + month * 0.02;
    const premiumFactor = 1 + month * 0.01;
  ...
  calculateTeamCosts(month) {
    const teamGrowthSchedule = [
      { month: 0, size: 2, avgSalary: 15e4 },
      // 2 founders
      { month: 1, size: 2, avgSalary: 15e4 },
      { month: 2, size: 2, avgSalary: 15e4 },
      { month: 3, size: 3, avgSalary: 145e3 },
      // Add 1 engineer
      { month: 4, size: 4, avgSalary: 14e4 },
      // Add 1 designer
      { month: 5, size: 5, avgSalary: 135e3 },
      // Add 1 advocate
      { month: 6, size: 6, avgSalary: 13e4 },
      // Add 1 sales
      { month: 7, size: 7, avgSalary: 125e3 },
      // Add 1 CS
      { month: 8, size: 8, avgSalary: 12e4 },
      // Add 1 more engineer
      { month: 9, size: 9, avgSalary: 115e3 },
      { month: 10, size: 10, avgSalary: 11e4 },
      { month: 11, size: 11, avgSalary: 105e3 },
      { month: 12, size: 12, avgSalary: 1e5 },
  ...
  calculateInfrastructureCosts(month) {
    const baseCost = 2e3;
    const scaleFactor = 1 + month * 0.08;
  ...
  calculateMarketingCosts(month) {
    const baseCost = 8e3;
    let growthFactor = 1 + month * 0.05;
    if (month > 12) {
      growthFactor *= Math.pow(0.95, month - 12);
    }
    const minCost = 3e3;
  ...
  calculateUnitEconomics() {
    ...
    this.unitEconomics = {
      ...
      // 10% viral coefficient from referrals
      networkEffectValue: 0.05,
      // 5% value from network effects
...
var model_default = FinancialModel;
