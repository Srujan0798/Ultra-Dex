...
let PolicyEngine = class extends EventEmitter {
  ...
  async evaluatePolicy(policyId, context) {
    ...
    if (!compliant) {
      ...
      if (
        this.enforcementLevel === 'block' ||
        (this.enforcementLevel === 'strict' && policy.severity === 'critical')
      ) {
        throw new Error(
          `Policy violation: ${policy.name} - ${violations.map((v) => v.message).join(', ')}`
        );
      }
    }
    return result;
...
PolicyEngine = __decorateClass([singleton()], PolicyEngine);
var policy_engine_default = PolicyEngine;
export { PolicyEngine, policy_engine_default as default };
