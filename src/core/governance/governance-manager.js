...
export class PolicyEngine {
    ...
    addPolicy({ id, name, description = '', scope = 'global', condition, enforcement = 'block' }) {
        this.policies.set(id, {
            id, name, description, scope, condition, enforcement,
            enabled: true, created: Date.now(), violationCount: 0,
        });
    }
    ...
    evaluate(context) {
        ...
        for (const [id, policy] of this.policies) {
            ...
            try {
                ...
                results.push({ policyId: id, policyName: policy.name, passed: false, error: error.message });
    ...
    getStats() {
        return {
            ...
            byPolicy: Object.fromEntries(
                [...this.policies.values()].map(p => [p.id, { name: p.name, violations: p.violationCount }])
            ),
        };
    }
}
...
export class ApprovalWorkflow extends EventEmitter {
     ...
     */
    requestApproval({ requestId = null, agentId, action, resource = null, reason = '', riskLevel = 'medium' }) {
    ...
    getStats() {
        ...
        const avgResponseTime = this.history.length > 0
            ? this.history.reduce((sum, r) => sum + (r.resolvedAt - r.requestedAt), 0) / this.history.length
            : 0;
        ...
        return {
            ...
            approvalRate: this.history.length > 0 ? Math.round(approved / this.history.length * 100) : 0,
