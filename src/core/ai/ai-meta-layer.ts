...
let AIMetaLayer = class {
  ...
  initializeProviders() {
    this.providers.set('mock', {
      client: (_model) => ({
        ...
        generateText: async (opts) => {
          ...
          if (
            (lastMessage.includes('Objective') || lastMessage.includes('Relevant')) &&
            !opts.messages.some((m) => m.role === 'tool')
          ) {
            return {
    ...
    this.activeProvider =
      this.providers.get(this.config.defaultProvider) || this.providers.values().next().value;
  }
  /**
  ...
  private async processBatch() {
    ...
    await Promise.all(
      currentBatch.map(async (req) => {
        try {
          const result = await this.executeCall(req.model, req.messages, {
            ...req.options,
            noBatch: true,
          });
          req.resolve(result);
    ...
    if (schema.paths) {
      return {
        paths: [
          ...
          {
            name: 'Path B: Microservices',
            description: 'Scalable but complex',
            steps: ['1', '2', '3'],
          },
...
export { AIMetaLayer, aiMetaLayer, ai_meta_layer_default as default };
