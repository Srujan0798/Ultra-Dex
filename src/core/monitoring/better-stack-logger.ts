...
export class BetterStackLogger {
  ...
  private createEntry(level: string, message: string, metadata?: Record<string, unknown>): LogEntry {
    return {
  ...
  // Track AI request
  aiRequest(provider: string, model: string, tokens: number, latencyMs: number, cost?: number): void {
