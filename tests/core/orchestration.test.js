...
describe('AgentOrchestrator', () => {
  ...
  it('should initialize and register tools', async () => {
    ...
    orchestrator.mcpServerFactory = async () => ({
      toolsMap: new Map([
        ['test-tool', { description: 'test', schema: {}, handler: async () => 'result' }],
      ]),
    });
  ...
  it('should block task execution via governance', async () => {
    ...
    await assert.rejects(() => orchestrator.executeTask('Dangerous task'), {
  ...
  it('should select correct agent based on capabilities', async () => {
    ...
    const agentId = orchestrator.selectAgentForTask('Build a button', {
