...
describe.skip(
  'Session-isolated TaskGraph execution (requires full system setup)',
  { timeout: 15000 },
  () => {
    test('executeNexus creates an isolated ExecutionContext per concurrent session', async () => {
      ...
      const orchestrator = new AgentOrchestrator({
        nexusExecutor: async (objective, options, runtimeOrchestrator, executionContext) => {
          ...
          assert.strictEqual(
            runtimeOrchestrator.activeSessions.has(executionContext.sessionId),
            true
          );
          ...
          return {
            ...
            taskObjectives: Array.from(executionContext.tasks.tasks.values()).map(
              (task) => task.objective
            ),
          };
        },
      ...
      assert.deepStrictEqual(startedContexts.map((ctx) => ctx.objective).sort(), [
        'alpha-objective',
        'beta-objective',
      ]);
