...
class MockAIRouter {
  async routeRequest() {
    return { text: 'Mock AI Output' };
  }
}
class MockRegistry {
  async getAgent() {
    return { execute: async () => 'Agent output' };
  }
}
...
describe('ExecutionEngine', () => {
  ...
  it('should handle step execution failure', async () => {
    ...
    await assert.rejects(() => engine.execute(task), { message: 'Step failed' });
    assert.strictEqual(task.status, 'failed');
    assert.strictEqual(task.errors.length, 1);
  });

  it('should yield progress updates in executeStream', async () => {
    const task = new ExecutionTask('t1', 'in', 'agent', [{ type: 'generate', id: 's1' }]);
