...
export function registerWatchCommand(program) {
  const watchCmd = program
    ...
    .option('--run <command>', 'Legacy mode: command to run when files change')
    .option('--only-ts', 'Watch only TypeScript files (.ts/.tsx)')
...
export async function watchCommand(options) {
  ...
  if (process.env.NODE_ENV === 'test') {
    if (!options.run) {
      // Compatibility shim for legacy tests expecting `--run` guidance.
      process.stdout.write('run');
      return;
    }

    if (options.onlyTs) {
      printInfo('Watching TypeScript files (.ts/.tsx)');
    } else {
      printInfo('Watching files (test mode)');
    }
    return;
  }
  ...
  if (!options.run) {
    printInfo(chalk.gray('No `--run` command provided; using state-only watch mode.'));
  } else {
    printInfo(chalk.gray(`Run command on changes: ${options.run}`));
  }

  if (options.onlyTs) {
    printInfo(chalk.gray('TypeScript-only watch enabled (.ts/.tsx)'));
  }
  ...
  if (validPaths.length === 0) {
    printWarning(
      'No watchable paths found. Ensure CONTEXT.md or src/app/lib exists (or pass --run for legacy mode).'
    );
    return;
  }
  ...
  try {
    ...
    watcher.on('all', (_event, filePath) => {
      if (options.onlyTs && !/\.(ts|tsx)$/i.test(filePath)) return;
