...
function displayResults(analysis) {
  ...
  logger.info('Confidence:', { detail: `${(confidence * 100).toFixed(1)}%` });
  logger.info('Timestamp:', { detail: new Date(analysis.timestamp).toLocaleString() });
  logger.spacer();

  if (predictions.length === 0) {
    logger.success('✨ No potential issues detected!');
    return;
  }
  ...
  if (critical.length > 0) {
    ...
    critical.forEach((p) => {
      logger.error(`• ${p.message}`, { context: path.basename(p.file) });
    });
    logger.spacer();
  }
  ...
  if (high.length > 0) {
    ...
    high.forEach((p) => {
      logger.warn(`• ${p.message}`, { context: path.basename(p.file) });
    });
    logger.spacer();
  }
  ...
  if (medium.length > 0) {
    ...
    medium.forEach((p) => {
      logger.info(`• ${p.message}`, { context: path.basename(p.file) });
    });
    logger.spacer();
  }

  if (low.length > 0) {
    logger.debug(`LOW (${low.length})`);
    low.forEach((p) => {
      logger.debug(`• ${p.message}`, { context: path.basename(p.file) });
    });
    logger.spacer();
  }
  ...
  logger.info(`Total findings: ${predictions.length}`);
  logger.info(
