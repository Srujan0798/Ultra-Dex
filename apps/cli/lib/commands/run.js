...
const TRACE_PERSISTENCE_WRAPPED = Symbol('tracePersistenceWrapped');
let tracePersistenceClient = null;
...
function isExecutionTracePersistenceEnabled() {
  return Boolean(process.env.DATABASE_URL);
}

async function resolvePostgresClientFactory() {
  if (postgresClientFactoryResolved) {
...
async function getTracePersistenceClient() {
  if (!isExecutionTracePersistenceEnabled()) {
    return null;
  }

  const clientFactory = await resolvePostgresClientFactory();
  if (!clientFactory) {
    return null;
  }

  if (!tracePersistenceClient) {
    tracePersistenceClient = clientFactory();
  }

  if (!tracePersistenceClientReady) {
    try {
      await tracePersistenceClient.init();
    } catch {
      return null;
    } finally {
      tracePersistenceClientReady = true;
    }
  }

  if (tracePersistenceClient.isFallbackMode()) {
    return null;
  }

  return tracePersistenceClient;
}

async function persistExecutionTraceStart(trace, { agent, task } = {}) {
  const client = await getTracePersistenceClient();
  if (!client || !trace?.runId) return;

  try {
    await client.query(
      `INSERT INTO execution_traces (run_id, user_id, agent, task, status)
       ...
       DO UPDATE SET
         ...
         status = 'running',
         completed_at = NULL`,
      [trace.runId, resolveTraceUserId(), String(agent || 'unknown'), String(task || '')]
    );
  } catch (error) {
    logRun('warn', 'run.trace.persistence_start_failed', {
      trace,
      module: 'run.trace.persistence',
      detail: error.message,
    });
  }
}

async function persistExecutionTraceStep(runId, stepEntry) {
  if (!runId || !stepEntry) return;
  const client = await getTracePersistenceClient();
  if (!client) return;
  ...
  try {
    await client.query(
      `UPDATE execution_traces
       ...
       WHERE run_id = $2`,
      [JSON.stringify([stepEntry]), runId]
    );
  } catch (error) {
    logRun('warn', 'run.trace.persistence_step_failed', {
      run_id: runId,
      module: 'run.trace.persistence',
      detail: error.message,
    });
...
async function markExecutionTraceCompleted(runId) {
  const client = await getTracePersistenceClient();
  ...
  try {
    ...
    logRun('warn', 'run.trace.persistence_complete_failed', {
      run_id: runId,
      module: 'run.trace.persistence',
      detail: error.message,
    });
...
async function markExecutionTraceFailed(runId) {
  const client = await getTracePersistenceClient();
  if (!client || !runId) return;
  ...
  try {
    ...
    logRun('warn', 'run.trace.persistence_fail_failed', {
      run_id: runId,
      module: 'run.trace.persistence',
      detail: error.message,
    });
...
function attachExecutionTracePersistence(trace) {
  if (!trace || typeof trace.record !== 'function') return;
  if (trace[TRACE_PERSISTENCE_WRAPPED]) return;

  const originalRecord = trace.record.bind(trace);
  trace[TRACE_PERSISTENCE_WRAPPED] = true;
  trace.record = async (...args) => {
    const entry = await originalRecord(...args);
    await persistExecutionTraceStep(trace.runId, entry);
    return entry;
  };
}
...
async function ensureTraceStarted(projectContext, agentId, task) {
  ...
  attachExecutionTracePersistence(trace);
  if (!trace.started) {
    trace.started = true;
    await persistExecutionTraceStart(trace, { agent: agentId, task });
    await trace.record({
      agent: agentId,
      action: 'RUN_START',
      input: task,
    ...
    .action(async (agentName, options) => {
      ...
      try {
        ...
        if (!trace.started) {
          ...
          trace.started = true;
          await persistExecutionTraceStart(trace, { agent: agentName, task });
          await trace.record({
            agent: agentName,
            action: 'RUN_START',
            input: task,
        ...
        await markExecutionTraceCompleted(trace?.runId);
        ...
        if (trace) {
          await markExecutionTraceFailed(trace.runId);
  ...
  program
    ...
    .action(async (feature, options) => {
      ...
      try {
        ...
        syncActiveRunId(trace.runId);
        attachExecutionTracePersistence(trace);
        await persistExecutionTraceStart(trace, { agent: 'planner', task: feature });
        printInfo(`Execution trace run_id: ${trace.runId}`);
        const maxSteps = resolveMaxSteps(options.maxSteps);
