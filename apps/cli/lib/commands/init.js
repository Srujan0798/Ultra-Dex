  ...
  program
    ...
    .action(async (objective, options) => {
      try {
        ...
        if (objective) {
          const isPathObjective =
            objective.startsWith('/') || objective.startsWith('./') || objective.startsWith('../');
          if (isPathObjective) {
            const outputDir = path.resolve(objective);
            await scaffoldSimpleDexProject(outputDir, {
              projectName: path.basename(outputDir),
              providers: ['nvidia'],
            });
            logger.success(`Project scaffolded at ${outputDir}`);
            return;
          }
...
async function handleInteractiveInit(options) {
  const { projectType } = await inquirer.prompt([
    {
      ...
      choices: [
        { name: 'Simple Ultra-Dex Script (Lightweight programmatic setup)', value: 'simple' },
        { name: 'Full SaaS Application (Next.js, Prisma, Auth, etc.)', value: 'saas' },
      ],
    },
  ]);
  ...
  if (projectType === 'simple') {
    const answers = await inquirer.prompt([
      {
        ...
        default: options.name || 'ultra-dex-project',
        validate: validateProjectName,
      },
      {
        type: 'checkbox',
        name: 'providers',
        message: 'Select AI Integration Protocols (Providers you have keys for):',
        choices: [
          { name: 'NVIDIA (Preferred)', value: 'nvidia' },
    ...
    logger.spacer();
    const spinner = ora(chalk.hex('#8b5cf6')('Assembling lightweight script matrix...')).start();
    ...
    logger.info('  2. npm install');
    logger.info('  3. Configure API keys in .env');
...
/**
 * Scaffold a simple Ultra-Dex project (SDK setup)
 * @param {string} outputDir - Target directory
 * @param {Object} answers - User choices
 */
async function scaffoldSimpleDexProject(outputDir, answers) {
  await fs.mkdir(outputDir, { recursive: true });
...
`;
  ...
  const packageJson = {
    ...
    dependencies: {
      '@ultra-dex/sdk': 'latest',
    },
  };

  const configJson = {
    ai: {
      defaultProvider: answers.providers?.[0] || 'nvidia',
    },
    governance: {
      enabled: true,
    },
  ...
  const indexTs = `import { UltraDex } from '@ultra-dex/sdk';
...
// Run a simple task
const result = await dex.run({
  agent: 'planner',
  task: 'Create a "Hello World" plan for AI orchestration',
});
