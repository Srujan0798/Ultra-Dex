...
export async function syncContextWithDiff(projectDir, reason = null) {
  ...
  // Update CONTEXT.md with timestamp before sync
  await updateContextTimestamp(contextPath);
...
/**
 * Update CONTEXT.md with current timestamp
 */
async function updateContextTimestamp(contextPath) {
  let content = '';
...
`;
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].startsWith('> Last updated:')) {
      lines[i] = timestampLine;
      foundTimestamp = true;
      break;
    }
  }

  // If no timestamp was found, add it after the first line (title)
  if (!foundTimestamp && lines.length > 0) {
    if (lines[0].startsWith('#')) {
      lines.splice(1, 0, timestampLine);
    } else {
...
export function registerSyncCommand(program) {
  const syncCmd = program
    ...
    .option(
      '--brain',
      'Auto-update CONTEXT.md from codebase analysis (eliminates human middleware)'
    )
    ...
    .action(async (options) => {
      try {
        ...
        if (options.jira || options.notion || options.trello) {
          if (options.jira) {
            try {
              ...
              printSuccess(
                chalk.green(
                  `✅ Jira sync prepared (${result.summary.epics} epics, ${result.summary.stories} stories)`
                )
              );
            } catch (error) {
        ...
        if (options.watch) {
          ...
          await startContextAutoSyncWatcher(
            projectDir,
            Number.isNaN(debounceMs) ? 800 : debounceMs
          );
          return;
        }
        ...
        printSuccess(
          chalk.green(
            `  ✅ Context Snapshot Complete (${syncResult?.summary?.fileCount || 0} Files scanned)`
          )
        );
        ...
        } else if (!options.jira && !options.notion && !options.trello) {
  ...
  syncCmd._examples = [
    ...
    {
      command: 'ultra-dex sync --pull --target backups/sync',
      description: 'Pull state from a custom sync target',
    },
    ...
    {
      command: 'ultra-dex sync --jira --project PROJ',
      description: 'Prepare Jira epic/story sync',
    },
...
async function handleBrainSync(projectDir) {
  ...
  printInfo(
    chalk.gray(`    Dependencies: ${graph.edges.filter((e) => e.type === 'depends_on').length}`)
  );
