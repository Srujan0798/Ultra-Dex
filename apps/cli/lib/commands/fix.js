...
export function registerFixCommand(program) {
  const fixCmd = program
    ...
    .option('--lint', 'Run lint-focused fixes')
    .option('--format', 'Run formatting-focused fixes')
    .option('--all', 'Run all available fixers')
    ...
    .action(async (options) => {
      try {
        ...
        if (options.dryRun) {
          printInfo(chalk.gray('Dry run mode enabled: no files will be modified.'));
        }
        if (options.lint) {
          printInfo(chalk.gray('lint fixer mode selected.'));
        }
        if (options.format) {
          printInfo(chalk.gray('format fixer mode selected.'));
        }
        if (options.all) {
          printInfo(chalk.gray('All fixers selected.'));
        }
