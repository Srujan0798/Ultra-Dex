...
export class PredictiveDebugger extends EventEmitter {
  ...
  async analyze() {
    ...
    if (analysis.predictions.length > 0) {
      analysis.confidence =
        analysis.predictions.reduce((sum, p) => sum + p.confidence, 0) /
    ...
    for (const filePath of this.watchedPaths) {
      try {
        ...
        const metrics = {
          ...
          cyclomaticIndicators: (content.match(/if\s*\(|else\s*if|switch\s*\(|\?\s*:/g) || [])
            .length,
        };

        // Flag high complexity files
        if (
          metrics.nestedBlocks > 10 ||
          metrics.longFunctions > 0 ||
          metrics.cyclomaticIndicators > 20
        ) {
          predictions.push({
            type: 'complexity',
            file: filePath,
  ...
  getHighRiskPredictions() {
    ...
    return latest.predictions.filter((p) => p.riskLevel === 'critical' || p.riskLevel === 'high');
  }
