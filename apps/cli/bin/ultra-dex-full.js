...
async function importUpdateNotifier(timeoutMs = 1500) {
  try {
    const module = await Promise.race([
      import('update-notifier'),
      new Promise((_, reject) => {
        setTimeout(() => reject(new Error('update_notifier_import_timeout')), timeoutMs);
      }),
    ]);
    return module.default;
  } catch (error) {
    logCli('warn', 'cli.update_check_skipped', { detail: error.message });
    return null;
  }
}
...
// Check for updates
if (!wantsHelp) {
  const updateNotifier = await importUpdateNotifier();
  if (updateNotifier) {
