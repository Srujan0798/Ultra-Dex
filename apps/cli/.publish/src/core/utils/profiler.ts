...
Total time: ${formatDuration(grandTotal)}`)
  ...
  logger.log(
    chalk.dim(`Operations profiled: ${labels.length}
`)
  );
}
  ...
  for (const [label, data] of Object.entries(stats)) {
    if (data.average > 1e3) {
      ...
      if (label.includes('fetch') || label.includes('api')) {
        suggestions.push(
          `\u2022 ${label}: Consider implementing request caching or parallelization`
        );
      }
    }
    if (data.count > 10 && data.average > 100) {
      suggestions.push(
        `\u2022 ${label}: Called ${data.count} times, consider batching or memoization`
      );
    }
  }
