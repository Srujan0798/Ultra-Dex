 ...
 * Bridges Unified Memory with Agent-specific needs.
 * Handles Hot/Warm/Cold tiering logic.
 */
...
export class AgentMemoryManager {
  ...
  async prune(fromTier, toTier, options = {}) {
    ...
    // 3. Move a percentage of items to toTier
   ...
   */
  async getTierStats() {
    // This would ideally be a single optimized query in UnifiedMemory
    const tiers = ['hot', 'warm', 'cold'];
    const result = {};

    for (const tier of tiers) {
      const items = await this.retrieve('', { tags: [tier], limit: 1000 });
