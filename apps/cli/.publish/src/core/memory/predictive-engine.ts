...
let PredictiveEngine = class {
  constructor(options = {}) {
    ...
    this.cache =
      options.cache instanceof ContextCache ? options.cache : options.cache || contextCache;
  ...
  analyzeTaskIntent(task) {
    ...
    const entities = uniqueStrings(normalizedTask.match(/\b[A-Z][a-zA-Z0-9_-]+\b/g) || []).slice(
      0,
      8
    );
    let intent = 'general';
    if (
      keywords.some((word) =>
        ['query', 'queries', 'schema', 'database', 'sql', 'prisma'].includes(word)
      )
    ) {
      intent = 'data';
    } else if (
...
PredictiveEngine = __decorateClass([singleton()], PredictiveEngine);
const predictiveEngine = new PredictiveEngine();
var predictive_engine_default = PredictiveEngine;
export { PredictiveEngine, predictive_engine_default as default, predictiveEngine };
