function normalizeResult(item, fallbackTier) {
  return {
    ...
    timestamp: item?.timestamp || item?.createdAt || /* @__PURE__ */ new Date().toISOString(),
  };
}
...
function createMemorySearchTool({ manager }) {
  return {
    name: 'memory-search',
...
var memory_search_default = createMemorySearchTool;
export { createMemorySearchTool, memory_search_default as default };
