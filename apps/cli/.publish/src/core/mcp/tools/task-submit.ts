...
function createTaskSubmitTool({ manager }) {
  return {
    name: 'task-submit',
    ...
    async handler({ task, priority = 'medium', agentPreference } = {}) {
      ...
      return {
        taskId,
        status: 'queued',
        estimatedStart: new Date(Date.now() + (manager.taskQueue.length - 1) * 3e4).toISOString(),
      };
    },
  };
}
var task_submit_default = createTaskSubmitTool;
export { createTaskSubmitTool, task_submit_default as default };
