...
import { getAuditDB } from './audit-db.js';
...
function toIso(ts: number | string | Date) {
...
function buildAuditPayload(entry: any = {}, timestamp = toIso(Date.now())) {
  const payload = {
    timestamp,
    ...entry,
  };
...
export function initializeGovernanceAuditSink() {
  ...
  logger.subscribe(
    ...
    async (event) => {
      ...
      const db = getAuditDB();
      const store: IAuditStore = db.getStore();
      await store.logEvent({
        action: payload.operation || payload.action || 'operation',
        agentId: payload.agent?.id || payload.agentId || 'unknown',
        task: payload.task || payload.resource || null,
        result: payload.allowed ? 'allowed' : 'blocked',
        outcome: payload.allowed ? 'allowed' : 'blocked',
        details: payload,
...
export async function logOperation(entry: any) {
...
export async function readAuditLog() {
  ...
  return events.map((e) => ({
    ...
    task: e.task,
    result: e.result,
    allowed: e.result === 'allowed' || e.outcome === 'allowed',
  }));
}

export async function generateComplianceReport({ since, until, writeToFile = false }: any = {}) {
  ...
  const summary: any = {
    ...
    totals: {
      ...
      allowed: filtered.filter((event: any) => event.allowed).length,
      blocked: filtered.filter((event: any) => !event.allowed).length,
    },
    byAgent: {},
    byOperation: {},
    blockedReasons: {},
  ...
  for (const event of filtered) {
    const agentId = event.agent?.id || event.agentId || 'unknown';
    ...
    const operation = event.operation || event.action || 'unknown';
...
export async function exportAuditLog({ format = 'json', since, until, outputPath }: any = {}) {
  const events = await readAuditLog();
  const start = since ? new Date(since).getTime() : null;
  const end = until ? new Date(until).getTime() : null;
