...
export async function loadState() {
  try {
    const content = await fs.readFile(
      path.resolve(process.cwd(), '.ultra/autonomous/state.json'),
      'utf8'
    );
...
export async function saveState(state) {
  const autonomousDir = path.resolve(process.cwd(), '.ultra/autonomous');
  const statePath = path.resolve(autonomousDir, 'state.json');
