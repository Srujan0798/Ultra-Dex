...
class VideoTutorialSystem {
  ...
  async listTutorials(filter = {}) {
    ...
    for (const tutorial of filteredTutorials) {
      ...
      console.log(
        `   Duration: ${chalk.blue(tutorial.duration)} | Level: ${this.formatLevel(tutorial.level)} | Tags: ${tutorial.tags.join(', ')}`
      );
  ...
  async getRecommendedTutorials(userProfile) {
      ...
      recommendations.sort((a, b) => {
        const aMatches = a.tags.filter((tag) =>
          userProfile.interests.some(
            (interest) => interestMap[interest] && interestMap[interest].includes(tag)
          )
        ).length;
    ...
    for (const tutorial of recommendations.slice(0, 5)) {
      ...
      console.log(`   ${chalk.gray(tutorial.description)}`);
      console.log(
        `   Duration: ${chalk.blue(tutorial.duration)} | Level: ${this.formatLevel(tutorial.level)}`
      );
      console.log(`   ${chalk.cyan(`URL: ${tutorial.url}`)}\n`);
    }
  }
...
export async function runTutorialSystem(action, options = {}) {
  ...
  switch (action) {
    ...
    default:
      console.log(
        chalk.yellow(
          'Available actions: list, recommended, search, by-level, by-tag, create-playlist'
        )
      );
  }
}
