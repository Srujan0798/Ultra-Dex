...
describe('Config Command', () => {
  ...
  describe('MCP Config Generation', () => {
    ...
    test('MCP config uses correct command', async () => {
      ...
      assert.ok(
        config.mcpServers['ultra-dex'].cwd.includes(path.basename(tmpDir)),
        'CWD should contain temp directory name'
      );
    });
  });
...
/**
 * Error handler for config-cmd.test
 * @param {Error} error - Error to handle
 */
function handleError(error) {
  try {
    console.error('[config-cmd.test]', error instanceof Error ? error.message : String(error));
  } catch (_) {
    // Fail silently
  }
}
