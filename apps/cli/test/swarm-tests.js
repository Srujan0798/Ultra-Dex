...
describe('Swarm System Integration Tests', () => {
  ...
  describe('Pipeline Execution', () => {
    it('should validate pipeline correctly', () => {
      ...
      assert(
        invalidResult.errors[0].error.includes('Depends on'),
        'Should identify dependency issue'
      );
    });
    ...
    it('should execute pipeline with timeout protection', async () => {
      ...
      const pipeline = [{ agent: 'planner', task: 'Simple planning task' }];
  ...
  describe('Agent Suggestion System', () => {
    it('should suggest appropriate agents for task descriptions', () => {
      const suggestions = coordinator.suggestAgents(
        'I need to create API endpoints for user management'
      );
      ...
      assert(
        perfSuggestions.includes('performance'),
        'Should suggest performance for speed issues'
      );
    });
    ...
    it('should rank suggestions by relevance', () => {
      const suggestions = coordinator.suggestAgents(
        'I need to create API endpoints and handle authentication'
      );
  ...
  describe('Swarm Planning Functionality', () => {
    it('should validate feature input', async () => {
      ...
      try {
        ...
        assert(
          error.message.includes('Feature parameter is required'),
          'Should validate empty feature'
        );
      }
      ...
      try {
        ...
        assert(
          error.message.includes('Feature parameter is required'),
          'Should validate non-string feature'
        );
      }
    });
  ...
  describe('Execution Order and Dependencies', () => {
    ...
    it('should validate execution order parameters', () => {
      ...
      assert.throws(
        ...
        /Each agent name must be a non-empty string/,
        'Should validate agent names'
      );
    });
  ...
  describe('Error Handling and Resilience', () => {
    it('should handle agent execution errors', async () => {
      ...
      const pipeline = [{ agent: 'nonexistent-agent', task: 'Should fail' }];
    ...
    it('should handle parallel execution errors', async () => {
      ...
      try {
        ...
        assert(
          error.message.includes('Parallel execution failed'),
          'Should identify parallel execution failure'
        );
      }
    });
  });
