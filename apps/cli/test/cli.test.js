...
test('--version returns valid semver', () => {
  ...
  // Extract version from output (may include performance messages)
  const versionMatch = output.match(/(\d+\.\d+\.\d+)$/);
  assert.ok(versionMatch, `Output should contain a version number: ${output}`);
  ...
  assert.equal(
    version,
    EXPECTED_VERSION,
    `Version should match package.json (${EXPECTED_VERSION}), got: ${version}`
  );
});
