    ...
    <main className="space-y-6">
      <section className="grid gap-4 md:grid-cols-3">
        {([
          ...
          <article key={item.key} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
        ...
        <div className="mt-4 h-2 rounded-full bg-slate-800">
          <div className="h-2 rounded-full bg-gradient-to-r from-rose-400 via-amber-400 to-blue-400" style={{ width: total > 0 ? '100%' : '0%' }} />
        </div>
        <p className="mt-3 text-sm text-slate-400">
          Total memory records: <span className="font-semibold text-slate-200">{total}</span>.
          Hot tier is optimized for instant recall, warm for active context, and cold for long-term ledger.
        </p>
      </section>
    </main>
  );
