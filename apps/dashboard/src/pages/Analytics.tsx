...
const MOCK_PROVIDERS = [
    { name: 'OpenAI GPT-4o', requests: 1247, tokens: 842300, cost: 12.63, avgLatency: 320, p95: 680, p99: 1200, errorRate: 0.003, status: 'healthy' },
    { name: 'Claude 3.5 Sonnet', requests: 983, tokens: 725100, cost: 10.88, avgLatency: 410, p95: 850, p99: 1400, errorRate: 0.005, status: 'healthy' },
    { name: 'Gemini 2.0 Flash', requests: 756, tokens: 538200, cost: 2.15, avgLatency: 180, p95: 340, p99: 620, errorRate: 0.001, status: 'healthy' },
    { name: 'Deepseek R1', requests: 412, tokens: 312000, cost: 0.94, avgLatency: 520, p95: 1100, p99: 2100, errorRate: 0.012, status: 'degraded' },
    { name: 'Mistral Large', requests: 298, tokens: 198400, cost: 1.59, avgLatency: 290, p95: 560, p99: 980, errorRate: 0.002, status: 'healthy' },
    { name: 'Llama 3.3 70B', requests: 187, tokens: 145200, cost: 0.29, avgLatency: 650, p95: 1400, p99: 2800, errorRate: 0.008, status: 'healthy' },
    { name: 'Grok 3', requests: 134, tokens: 98600, cost: 1.48, avgLatency: 380, p95: 720, p99: 1300, errorRate: 0.004, status: 'healthy' },
];
...
function StatCard({ label, value, sub, accent = 'emerald' }: { label: string; value: string; sub?: string; accent?: string }) {
                    ...
                    <tbody>
                        {providers.map((p) => (
                            <tr key={p.name} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                                ...
                                <td className="py-3 pr-4 text-right tabular-nums text-slate-300">{p.requests.toLocaleString()}</td>
                                <td className="py-3 pr-4 text-right tabular-nums text-slate-300">{p.avgLatency}ms</td>
                                ...
                                <td className="py-3 pr-4 text-right tabular-nums text-slate-300">${p.cost.toFixed(2)}</td>
                                <td className="py-3 text-center">
                                    <span className={`inline-block h-2.5 w-2.5 rounded-full ${p.status === 'healthy' ? 'bg-emerald-400' :
                                        p.status === 'degraded' ? 'bg-amber-400' : 'bg-red-400'
                                        }`} />
                                </td>
                            </tr>
                        ))}
                ...
                {providers.map((p) => (
                    <div key={p.name}>
                        <div className="mb-1 flex items-center justify-between text-xs">
                            ...
                            <span className="tabular-nums text-slate-500">p50 {p.avgLatency}ms · p95 {p.p95}ms · p99 {p.p99}ms</span>
                        </div>
                        <div className="flex gap-1">
                            <div className="h-3 rounded-l bg-emerald-500/60" style={{ width: `${(p.avgLatency / maxP99) * 100}%` }} />
                            <div className="h-3 bg-amber-500/50" style={{ width: `${((p.p95 - p.avgLatency) / maxP99) * 100}%` }} />
                            <div className="h-3 rounded-r bg-red-500/40" style={{ width: `${((p.p99 - p.p95) / maxP99) * 100}%` }} />
                        </div>
                    </div>
                ))}
            </div>
            <div className="mt-4 flex gap-4 text-[10px] text-slate-500">
                <span className="flex items-center gap-1"><span className="inline-block h-2 w-3 rounded bg-emerald-500/60" />Avg</span>
                <span className="flex items-center gap-1"><span className="inline-block h-2 w-3 rounded bg-amber-500/50" />P95</span>
                <span className="flex items-center gap-1"><span className="inline-block h-2 w-3 rounded bg-red-500/40" />P99</span>
            </div>
        </div>
    );
}

function BudgetTracker({ providers, budget }: { providers: typeof MOCK_PROVIDERS; budget: number }) {
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-5">
            ...
            <div className="mb-3 flex items-end justify-between">
                ...
                <div className="text-right">
                    <div className="text-lg font-semibold text-slate-300">{(totalTokens / 1_000_000).toFixed(2)}M</div>
...
function RouterStatus({ strategy, providers }: { strategy: string; providers: typeof MOCK_PROVIDERS }) {
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-5">
            ...
            <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-semibold text-emerald-300">{healthyCount}/{providers.length}</span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-slate-500">
                        {providers.sort((a, b) => a.avgLatency - b.avgLatency).slice(0, 3).map((p) => p.name.split(' ')[0]).join(' → ')}
                    </span>
                </div>
            </div>
        </div>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-4 gap-4">
                <StatCard label="Total Requests" value={totalRequests.toLocaleString()} sub="Last 24 hours" accent="emerald" />
                <StatCard label="Avg Latency" value={`${avgLatency}ms`} sub="Weighted by request volume" accent="cyan" />
                <StatCard label="Total Tokens" value={`${(totalTokens / 1_000_000).toFixed(2)}M`} sub="Prompt + completion" accent="violet" />
                <StatCard label="Total Cost" value={`$${totalCost.toFixed(2)}`} sub={`$${BUDGET_LIMIT.toFixed(2)} budget`} accent="amber" />
            </div>
