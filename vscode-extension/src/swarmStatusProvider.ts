...
export class SwarmStatusProvider implements vscode.TreeDataProvider<SwarmItem> {
    ...
    getChildren(element?: SwarmItem): Thenable<SwarmItem[]> {
        ...
        const statusItem = new SwarmItem(
            ...
            this.swarmStatus.status === 'running' ? vscode.TreeItemCollapsibleState.Expanded : vscode.TreeItemCollapsibleState.None,
            {
        ...
        if (this.swarmStatus.agents.length > 0) {
            items.push(new SwarmItem('Active Agents', vscode.TreeItemCollapsibleState.Expanded, undefined, 'agents'));
