...
import {
  createAlignmentStatusBar,
  updateAlignmentStatusBar,
  refreshAlignmentStatusBar,
} from './statusBar';
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(ContextPanelProvider.viewType, contextPanelProvider)
  );

  // Register Sidebar webview provider
  const sidebarProvider = new SidebarProvider(rootPath);
  context.subscriptions.push(
  ...
  // Welcome message
  vscode.window
    .showInformationMessage('Ultra-Dex v2 is now active!', 'View Commands', 'Open Dashboard')
    .then((selection) => {
...
function registerAllCommands(
  ...
  const commands = [
    ...
    vscode.commands.registerCommand('ultra-dex.runAgent', (agent: string, task: string) =>
      runAgent(agent, task)
    ),
    vscode.commands.registerCommand('ultra-dex.stopAgent', (agentItem: any) =>
      stopAgent(agentItem)
    ),
    ...
    vscode.commands.registerCommand('ultra-dex.runSpecificAgent', (agentItem: any) =>
      runSpecificAgent(agentItem)
    ),
    ...
    vscode.commands.registerCommand('ultra-dex.showAlignmentDetails', () =>
      showAlignmentDetailsCmd()
    ),
    ...
    vscode.commands.registerCommand('ultra-dex.refreshContext', () =>
      contextPanelProvider.refresh()
    ),
    ...
    vscode.commands.registerCommand('ultra-dex.showTaskDetails', (task: any) =>
  ...
  try {
    vscode.window.withProgress(
      ...
      async () => {
        ...
        vscode.window
          .showInformationMessage(`${agent} completed successfully`, 'View Results')
          .then((selection) => {
  ...
  try {
    vscode.window.withProgress(
      ...
      async () => {
        ...
        vscode.window
          .showInformationMessage('Swarm execution completed', 'View Dashboard', 'View Results')
          .then((selection) => {
    ...
    try {
      vscode.window.withProgress(
        ...
        async (progress) => {
          ...
          vscode.window
            .showInformationMessage('Implementation plan generated!', 'Open Plan', 'Start Building')
            .then((selection) => {
...
async function checkAlignment() {
  try {
    ...
    vscode.window
      .showInformationMessage(`Alignment Score: ${score}%`, 'View Details')
      .then((selection) => {
  ...
  try {
    vscode.window.withProgress(
      ...
      async () => {
        ...
        vscode.window
          .showInformationMessage('Execution complete', 'View Output')
          .then((selection) => {
...
async function reviewCode() {
  ...
  const task = selection ? `Review this code:\n\n${selection}` : `Review ${fileName}`;
