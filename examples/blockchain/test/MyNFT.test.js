...
describe("MyNFT", function () {
  ...
  describe("Minting", function () {
    ...
    it("Should prevent minting without payment", async function () {
      await expect(myNFT.connect(addr1).mint("uri.json"))
    ...
    it("Should refund excess payment", async function () {
      ...
      expect(finalBalance).to.be.gt(initialBalance - mintPrice - excess - ethers.parseEther("0.001"));
    });
    ...
    it("Should prevent minting over max per wallet", async function () {
      ...
      await expect(myNFT.connect(addr1).mint("extra.json", { value: mintPrice }))
    ...
    it("Should prevent minting when disabled", async function () {
      ...
      await expect(myNFT.connect(addr1).mint("uri.json", { value: mintPrice }))
  ...
  describe("Admin Functions", function () {
    ...
    it("Should prevent setting max supply below current", async function () {
      ...
      await expect(myNFT.setMaxSupply(0))
    ...
    it("Should prevent non-owner from admin functions", async function () {
      await expect(myNFT.connect(addr1).setMintPrice(100))
        .to.be.revertedWithCustomError(myNFT, "OwnableUnauthorizedAccount");
      
      await expect(myNFT.connect(addr1).toggleMinting())
        .to.be.revertedWithCustomError(myNFT, "OwnableUnauthorizedAccount");
