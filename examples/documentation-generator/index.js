...
class DocumentationGenerator {
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory()) {
          structure.children.push(
            await this.analyzeProjectStructure(fullPath, depth + 1, maxDepth)
          );
        } else {
          structure.children.push({
            path: fullPath,
  ...
  getStats() {
    ...
    const totalLOC = this.documentation.reduce(
      (sum, doc) => sum + (doc.metadata.linesOfCode || 0),
      0
    );
    ...
    return {
      ...
      generatedSince:
        this.documentation.length > 0 ? this.documentation[0].createdAt : new Date().toISOString(),
...
async function main() {
  ...
  try {
    ...
    console.log(
      'Documentation generator initialized. Use generateFromCode() to generate docs from actual files.'
    );
