...
1.  **Named Exports Only**: `export const foo = ...` (No `default export` amnesia).
2.  **JSDoc Everything**: If it's exported, it gets a TSDoc comment.
3.  **Zod Validation**: Trust nothing. Validate everything at the runtime boundary.
4.  **Error Boundaries**: No white screens of death. Catch it, log it, show a toast.
