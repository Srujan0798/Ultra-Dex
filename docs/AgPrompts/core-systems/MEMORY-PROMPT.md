...
| **Vector & Graph** | Semantic search finds *what*; Graph finds *why*. Use both. |
...
- **Your Job:** Retrieve only the *relevant* sliver of memory.
...
- **Don't just store logs.** Store *patterns*.
- **Don't just store code.** Store *architecture*.
