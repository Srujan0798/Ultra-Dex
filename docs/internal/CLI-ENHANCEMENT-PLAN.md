...
export function showCommandsTable(commands) {
  ...
  const rows = commands.map((c) => [chalk.cyan(c.name), chalk.dim(c.description)]);
  console.log(createTable(headers, rows));
}
```
...
export function showHelp() {
  ...
  console.log(
    `  ${chalk.dim('Run')} ${chalk.cyan('ultra-dex <command> --help')} ${chalk.dim('for details')}`
  );
  console.log('');
}
```
...
_Implementation ready for execution_
