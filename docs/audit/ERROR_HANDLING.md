...
- **Stream Error Handling**: The `Router`'s `streamChat` generator wraps the provider's stream in a `try/catch` block. If an error occurs mid-stream, it throws the error, terminating the generator. It does _not_ attempt to fallback mid-stream (which is reasonable but worth noting).
...
2. **Unified Timeout**: Ensure `Router` uses the same timeout mechanism as `BaseProvider` to avoid race conditions or double-timeout logic. Currently, `Router` wraps the call in _another_ timeout, which is redundant if `BaseProvider` already enforces one.
