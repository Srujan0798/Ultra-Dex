...
async function validateNamingConvention() {
  ...
  console.log(
    `   Found ${filesWithUnderscores.length} files with underscores (these are in archive and won't affect main docs)`
  );
  if (filesWithUnderscores.length > 0) {
    console.log(
      `   Sample: ${filesWithUnderscores
        .slice(0, 3)
...
async function main() {
  ...
  try {
    ...
    console.log('====================');
    console.log(
      `Naming Convention: ${namingResults.total} files with underscores found (mostly in archive)`
    );
    ...
    const totalIssues =
      versionResults.outdated + completenessResults.minimal + criticalResults.missing.length;
