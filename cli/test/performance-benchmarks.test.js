...
test('Performance: Cache effectiveness', async (t) => {
  ...
  await t.test('Cache provides performance benefit', () => {
    ...
    if (firstDuration > 0) {
      ...
      assert.ok(speedupRatio > 1.5, `Cache should provide speedup, got ratio: ${speedupRatio.toFixed(2)}x`);
    }
  });
});
...
test('Performance: Memory usage during graph operations', async (t) => {
  ...
  await t.test(`Memory usage increase is reasonable: ${(memDiff / 1024 / 1024).toFixed(2)} MB`, () => {
    // Memory increase should be reasonable (less than 100MB for typical operations)
    assert.ok(memDiff < 100 * 1024 * 1024, `Memory increase too high: ${(memDiff / 1024 / 1024).toFixed(2)} MB`);
  });
});
