...
`;
      ...
      assert.ok(enhanced.includes('## Project Statistics'), 'Should add Project Statistics section');
      assert.ok(enhanced.includes('| Files | 42 |'), 'Should include files in table');
      assert.ok(enhanced.includes('| Dependencies | 156 |'), 'Should include dependencies in table');
    });
    ...
    test('includes active phases when available', async () => {
      ...
      const state = {
        phases: [
          { name: 'Phase 1', status: 'in-progress', steps: [{ status: 'completed' }, { status: 'pending' }] },
...
`;
    ...
    test('handles empty phases gracefully', async () => {
      ...
      assert.ok(!enhanced.includes('### Active Phases:'), 'Should not include Active Phases when empty');
    });
    ...
    test('appends context when Current Focus section missing', async () => {
      ...
      assert.ok(enhanced.includes('### Current State'), 'Should add Current State within Current Focus');
    });
  ...
  describe('brainCommand functionality', () => {
    test('creates CONTEXT.md if it does not exist', async () => {
      ...
      try {
        ...
        const contextExists = await fs.access(contextPath)
          .then(() => true)
          .catch(() => false);
  ...
  describe('registerBrainCommand', () => {
    ...
    test('registers commit option', async () => {
      ...
      assert.ok(commitOption.description.includes('commit'), 'Should describe commit functionality');
    });
