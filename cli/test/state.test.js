...
import { withStateLock, loadState, saveState, updateState, computeState } from '../lib/commands/state.js';
...
describe('state management utilities', () => {
  ...
  describe('saveState', () => {
    ...
    test('creates .ultra directory if needed', async () => {
      ...
      const ultraDirExists = await fs.access(path.join(tmpDir, '.ultra'))
        .then(() => true)
        .catch(() => false);
