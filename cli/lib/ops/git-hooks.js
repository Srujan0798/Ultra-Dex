...
TODO_COUNT=$(git diff --cached --name-only | xargs grep -n "TODO\\|FIXME\\|BUG" 2>/dev/null | wc -l)

if [ $TODO_COUNT -gt 0 ]; then
  echo "⚠️  Found $TODO_COUNT TODO/FIXME/BUG comments in staged files."
...
class GitHookManager {
  ...
  async installHooks(options = {}) {
    ...
    printInfo(chalk.gray('- To bypass a hook: git commit --no-verify (not recommended in production)'));
  }
...
`;
  ...
  async listHooks() {
    ...
    try {
      ...
      const installedHooks = files.filter(file => 
        GIT_HOOKS.includes(file) && 
        (await fs.stat(path.join(this.hooksDir, file))).isFile()
      );
      ...
      for (const hook of installedHooks) {
        ...
        const execStatus = isExecutable ? chalk.green('executable') : chalk.yellow('not executable');
...
export function registerGitHookCommand(program) {
  ...
  hookCmd
    ...
    .action(async () => {
      try {
        ...
        printInfo(chalk.gray(`Installed Hooks: ${status.installed.length}/${status.totalAvailable}`));
  ...
  hookCmd._examples = [
    ...
    { command: 'ultra-dex githooks install --pre-commit', description: 'Install only pre-commit hook' },
