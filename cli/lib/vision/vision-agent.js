...
export class VisionAgent {
  constructor() {
    this.supportedFrameworks = [
      ...
      'remix', 'sveltekit', 'flutter', 'swiftui', 'compose'
    ];
  ...
  async validateImageFile(imagePath) {
    try {
      ...
      if (!this.imageFormats.includes(ext)) {
        throw new AppError(`Unsupported image format: ${ext}. Supported: ${this.imageFormats.join(', ')}`);
      }
...
Output only the code with no explanations unless specifically asked.`;
    ...
    // Create user prompt
    const userPrompt = customPrompt || `Convert this UI screenshot into ${framework} code. Generate the complete component with proper structure, styling, and functionality.`;
    ...
    try {
      ...
      if (error.response) {
        throw new AppError(`Vision API error: ${error.response.data.error?.message || error.response.statusText}`);
      }
