...
export class BillingSystem {
  ...
  async logUsage(agentName, provider, model, usage, metadata = {}) {
    ...
    const costInfo = this.calculateCost(provider, model, usage.inputTokens || 0, usage.outputTokens || 0);
  ...
  async getUsageReport(options = {}) {
    ...
    if (startDate) {
      filteredUsage = filteredUsage.filter(record => new Date(record.timestamp) >= new Date(startDate));
    }
    
    if (endDate) {
      filteredUsage = filteredUsage.filter(record => new Date(record.timestamp) <= new Date(endDate));
    }
    ...
    .action(async (options) => {
      try {
        ...
        if (options.status) {
          ...
          printInfo(`Daily: $${status.daily.spent.toFixed(4)}/$${status.daily.budget.toFixed(2)} (${status.daily.percentage.toFixed(1)}%)`);
          printInfo(`Monthly: $${status.monthly.spent.toFixed(4)}/$${status.monthly.budget.toFixed(2)} (${status.monthly.percentage.toFixed(1)}%)`);
