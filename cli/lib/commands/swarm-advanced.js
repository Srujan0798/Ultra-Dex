...
import { CheckpointManager, ConflictResolver, CostTracker, ProgressReporter } from '../lib/swarm/orchestrator.js';
...
export function showCostReport(costTracker) {
    ...
    if (report.byProvider.length > 0) {
        ...
        report.byProvider.forEach(({ provider, costFormatted, input, output }) => {
            console.log(chalk.gray(`  ${provider}: ${costFormatted} (${input} in / ${output} out tokens)`));
        });
    }
}
