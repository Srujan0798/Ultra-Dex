...
async function runAutoFixes(projectDir) {
    ...
    const missingFiles = ['QUICK-START.md', 'IMPLEMENTATION-PLAN.md', 'CONTEXT.md', '.gitignore', '.env.example'];
    for (const file of missingFiles) {
        ...
        try {
            ...
            if (file === '.gitignore') content = 'node_modules\n.env\n.ultra-dex/logs\n.ultra-dex/state.lock\n';
            if (file === '.env.example') content = 'OPENAI_API_KEY=\nANTHROPIC_API_KEY=\nGEMINI_API_KEY=\n';
...
async function runStructureAudit(ctx) {
    ...
    for (const check of checks) {
        ...
        if (exists) {
            ...
            ctx.results.push({ category: 'Structure', status: '✅', item: check.desc, points: `+${check.points}` });
            printSuccess(`  ✅ ${check.file} present`);
        } else {
            ...
            ctx.results.push({ category: 'Structure', status, item: `${check.desc} (Missing)`, points: '0' });
            if (status === '❌') printError(`  ❌ ${check.file} missing`);
            else printWarning(`  ⚠️  ${check.file} missing (recommended)`);
        }
    }
}
...
async function runDocumentationAudit(ctx) {
    const docs = [
        { file: 'README.md', desc: 'Main Readme', points: 10, sections: ['installation', 'usage', 'features'] },
        { file: 'QUICK-START.md', desc: 'Onboarding Guide', points: 10, sections: ['idea', 'tech stack', 'mvp'] },
        { file: 'CONTEXT.md', desc: 'Project Memory', points: 5, sections: ['purpose', 'state'] },
        { file: 'IMPLEMENTATION-PLAN.md', desc: 'Execution Path', points: 5, sections: ['phase', 'step'] }
    ];
    ...
    for (const doc of docs) {
        ...
        try {
            ...
            if (content.trim().length > 50) {
                ...
                ctx.results.push({ category: 'Docs', status: '✅', item: doc.desc, points: `+${doc.points}` });
                ...
                for (const section of doc.sections) {
                    ...
                    if (content.toLowerCase().includes(section.toLowerCase())) {
                        ctx.score += 2;
                        ctx.results.push({ category: 'Docs', status: '✅', item: `Section: ${section} in ${doc.file}`, points: '+2' });
                    } else {
                        ctx.results.push({ category: 'Docs', status: '⚠️', item: `Missing section: ${section} in ${doc.file}`, points: '0' });
                    }
                }
            } else {
                ctx.results.push({ category: 'Docs', status: '⚠️', item: `${doc.desc} is too short`, points: '0' });
                printWarning(`  ⚠️  ${doc.file} is too sparse`);
            }
        } catch {
            ctx.results.push({ category: 'Docs', status: '❌', item: `${doc.desc} (Missing)`, points: '0' });
...
function displayResults(ctx) {
    ...
    if (percentage >= 90) { grade = 'A'; gradeColor = chalk.green; message = 'Excellent! Your project is production-ready.'; }
    else if (percentage >= 75) { grade = 'B'; gradeColor = chalk.green; message = 'Good! Solid foundation, a few minor gaps.'; }
    else if (percentage >= 60) { grade = 'C'; gradeColor = chalk.yellow; message = 'Fair. Address highlighted issues.'; }
