...
// Clear previous log
fs.writeFileSync(LOG_FILE, `STRICT EXECUTION MODE: ACTIVE
...
function analyzeFile(filePath) {
    ...
    try {
        ...
        if (filePath.endsWith('.json')) {
            try { JSON.parse(content.replace(/\/\/.*$/mg, '')); } // rough jsonc support
            catch { 
                // Only fail if strict JSON
                if (!filePath.includes('tsconfig') && !filePath.includes('settings.json')) stats.status = 'INVALID_JSON'; 
            }
        }
