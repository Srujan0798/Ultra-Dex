...
`;
                ...
                if (!content.includes('useMemo')) {
                    const useMemoImport = content.includes('from \'react\'') || content.includes('from "react"');
                    if (useMemoImport) {
                        // Add useMemo to existing react import
                        if (content.includes('{ ') && (content.includes('from \'react\'') || content.includes('from "react"'))) {
                            if (!content.includes('useMemo')) {
...
function findImportEnd(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].startsWith('import ') || lines[i].startsWith('import{') || /^import\s/.test(lines[i])) {
            lastImport = i;
        }
    }
