...
/**
 * Agent Templates — Pre-built agent configurations for instant deployment
 *
 * The #1 barrier to AI agent adoption is "where do I start?" This module
 * solves the cold start problem with production-tested agent templates
 * that work out of the box.
 *
 * Features:
 *   1. Template Registry   — searchable catalog of agent configs
 *   2. Template Builder     — compose custom templates from primitives
 *   3. Built-in Templates   — 10+ ready-to-deploy agent templates
 *   4. Template Validation  — verify configs before deployment
 *   5. Template Versioning  — track and rollback template changes
 *
...
export function validateTemplate(template) {
  ...
  if (!template.role) errors.push('Missing role');
  if (!template.model) errors.push('Missing model');
  if (!template.systemPrompt) errors.push('Missing systemPrompt');
...
// ───────────────────────────────────────────────────────────────────────────
// Built-in Templates — Production-tested agent configurations
// ───────────────────────────────────────────────────────────────────────────
...
export const builtInTemplates = [
  {
    ...
    model: 'gpt-4o',
    systemPrompt:
      'You are a senior code reviewer. Analyze code for bugs, security vulnerabilities, performance issues, and style consistency. Provide actionable feedback with specific line references. Be thorough but constructive.',
    ...
    config: { maxTokensPerReview: 4000, confidenceThreshold: 0.7 },
  ...
  },
  {
    ...
    systemPrompt:
      'You are a technical writer. Generate clear, comprehensive documentation from source code. Include usage examples, API references, and architecture explanations. Use markdown formatting.',
  ...
  },
  {
    ...
    model: 'gpt-4o',
    systemPrompt:
      'You are a test engineering specialist. Generate comprehensive test suites including edge cases, error paths, and integration scenarios. Use the testing framework already present in the project. Aim for high coverage.',
  ...
  },
  {
    ...
    model: 'gemini-2.0-flash',
    systemPrompt:
      'You are a research analyst. Search for information, cross-reference sources, and synthesize findings into clear, well-cited reports. Always verify claims from multiple sources. Flag uncertainty levels.',
  ...
  },
  {
    ...
    model: 'gemini-2.0-flash',
    systemPrompt:
      'You are a data engineering agent. Process, validate, and transform data according to defined schemas. Report data quality issues. Handle errors gracefully with retry logic.',
  ...
  },
  {
    ...
    model: 'gpt-4o-mini',
    systemPrompt:
      'You are a helpful customer support specialist. Answer questions accurately using the knowledge base. If unsure, escalate to a human. Be empathetic, concise, and professional. Never make up information.',
  ...
  },
  {
    ...
    model: 'gpt-4o',
    systemPrompt:
      'You are a deployment operations agent. Execute deployments following strict procedures. Always verify pre-conditions, run health checks post-deployment, and be ready to rollback. Never bypass approval for production.',
    ...
    config: { requireApproval: true, rollbackOnFailure: true, healthCheckTimeout: 30000 },
  ...
  },
  {
    ...
    model: 'gpt-4o',
    systemPrompt:
      'You are a security auditor. Scan code for OWASP Top 10 vulnerabilities, check dependency versions, review configurations for security misconfigurations. Classify findings by severity (critical/high/medium/low). Provide remediation steps.',
  ...
  },
  {
    ...
    model: 'gpt-4o',
    systemPrompt:
      'You are a multi-agent coordinator. Break complex tasks into subtasks, assign them to appropriate specialized agents, monitor progress, and synthesize results. Handle conflicts between agent outputs using consensus.',
  ...
  },
  {
    ...
    model: 'gemini-2.0-flash',
    systemPrompt:
      'You are a knowledge curator. Organize information into structured, searchable knowledge bases. Create taxonomies, detect duplicates, maintain quality, and surface related knowledge. Build semantic links between concepts.',
...
// ───────────────────────────────────────────────────────────────────────────
// Template Registry — Searchable catalog
// ───────────────────────────────────────────────────────────────────────────
...
export class TemplateRegistry extends EventEmitter {
  constructor() {
    ...
    this.versions = new Map(); // templateId → version[]
  }
  ...
  register(template) {
    ...
    if (existing) {
      // Version the old one
      if (!this.versions.has(template.id)) this.versions.set(template.id, []);
  ...
  search({ category, tag, capability, query } = {}) {
    ...
    if (capability) results = results.filter((t) => t.capabilities?.includes(capability));
    if (query) {
...
// ───────────────────────────────────────────────────────────────────────────
// Template Builder — Compose custom templates from primitives
// ───────────────────────────────────────────────────────────────────────────
...
export default { TemplateRegistry, TemplateBuilder, validateTemplate, builtInTemplates };
