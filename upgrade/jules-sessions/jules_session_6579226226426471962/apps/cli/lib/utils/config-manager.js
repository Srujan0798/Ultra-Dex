...
class ConfigManager {
  ...
  validate() {
    ...
    if (
        typeof this.config.ai.timeout !== 'number' ||
        this.config.ai.timeout < 0
    ) {
        errors.push('ai.timeout must be a non-negative number');
    }
  ...
  /**
   * Get the current configuration object (redacted)
   */
  getConfig() {
    if (!this.loaded) {
      throw new Error('Configuration not loaded. Call load() first.');
    }
    return this.redact(this.config);
  }

  /**
   * Get the raw configuration object (including secrets)
   * Internal use only
   */
  getRawConfig() {
    if (!this.loaded) {
  ...
  redact(config) {
    ...
    const redactKeys = ['apiKey', 'token', 'secret', 'password', 'key', 'auth'];
    ...
    const recurse = (obj) => {
      ...
      for (const k in obj) {
        if (typeof obj[k] === 'object' && obj[k] !== null) {
          recurse(obj[k]);
        } else if (redactKeys.some(rk => k.toLowerCase().includes(rk))) {
           obj[k] = '********';
        }
      }
    };
  ...
  export() {
    ...
    const exported = this.redact(this.config);
