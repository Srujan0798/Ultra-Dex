...
export async function createProvider(providerId, options = {}) {
  ...
  if (agent) {
    await enforceAgentExecution({ agent, providerId });
  }
  ...
  if (!apiKey && providerConfig.envKey) {
     // Try to get from config
     apiKey = configManager.get(`ai.apiKeys.${providerId}`);
  }
...
export function getDefaultProvider() {
  ...
  // Check config first
  const configDefault = configManager.get('ai.defaultProvider');
  ...
  return configDefault || null; // Fallback to config default even if provider logic didn't validate it (it might work if no keys needed?)
}
...
export function checkConfiguredProviders() {
  return Object.entries(PROVIDERS).map(([id, config]) => ({
    ...
    name: config.name,
    envKey: config.envKey,
    configured: !!process.env[config.envKey] || !!configManager.get(`ai.apiKeys.${id}`),
  }));
}
