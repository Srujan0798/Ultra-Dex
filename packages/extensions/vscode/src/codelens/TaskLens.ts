  ...
  private parseTasks(content: string) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (taskMatch) {
        ...
        const functionMatch = title.match(
          /(?:function|method|class)\s+(\w+)|(\w+)\s*(?:function|method)/i
        );
        if (functionMatch) {
...
export async function showTaskDetails(task: TaskInfo) {
  const items = [`Section: ${task.section}`, `Status: ${task.status}`, `Line: ${task.line + 1}`];
...
export async function updateTaskStatus(
  ...
  vscode.window
    .showInformationMessage(`Task marked as ${statusText}: ${task.title}`, 'Open Plan')
    .then((selection) => {
