import type { GraphNode } from './types.js';
import type { ExecutionContext } from './executionAdapter.js';

export interface Decision {
  type: 'allow' | 'block' | 'pause';
  reason?: string;
}

export interface Rule {
  condition: (node: GraphNode, context: ExecutionContext) => boolean;
  action: 'allow' | 'block' | 'pause';
  reason?: string;
}

function evaluateRules(rules: Rule[], node: GraphNode, context: ExecutionContext): Decision {
  for (const rule of rules) {
    if (rule.condition(node, context)) {
      return { type: rule.action, reason: rule.reason };
    }
  }
  return { type: 'allow' };
}

export class ExecutionBlockedError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'ExecutionBlockedError';
  }
}

export class GovernanceManager {
  private auditLog: { timestamp: string; node: string; decision: string; reason: string }[] = [];

  constructor(private rules: Rule[] = []) {}

  addRule(rule: Rule): void {
    this.rules.push(rule);
  }

  evaluate(node: GraphNode, context: ExecutionContext): Decision {
    const decision = evaluateRules(this.rules, node, context);
    this.logDecision(node, decision);
    return decision;
  }

  private logDecision(node: GraphNode, decision: Decision): void {
    this.auditLog.push({
      timestamp: new Date().toISOString(),
      node: node.id,
      decision: decision.type,
      reason: decision.reason || '',
    });
  }

  getAuditLogs() {
    return [...this.auditLog];
  }
}
