...
dex.enableRouter({
  ...
  budgetLimit: 1.0, // Stop if total cost exceeds $1.00
  costPerToken: {
...
for (const [name, stats] of Object.entries(allStats)) {
  ...
  if (requestCount > 0) {
    console.log(
      `  ${name.padEnd(10)} │ ${requestCount} reqs │ avg ${avgLatency}ms │ p95 ${p95}ms │ err ${(errorRate * 100).toFixed(1)}% │ $${totalCost.toFixed(6)} │ circuit: ${circuitState}`
    );
  }
}
