...
function createMockProvider(overrides = {}) {
    return {
        async chat(messages, opts) {
            return { role: 'assistant', content: 'Hello!', usage: { promptTokens: 10, completionTokens: 20 } };
        },
...
function createFailingProvider(failCount = 1) {
    ...
    return {
        async chat(messages, opts) {
            ...
            return { role: 'assistant', content: 'Recovered!', usage: { promptTokens: 5, completionTokens: 10 } };
        },
        async *stream() { yield { delta: 'x' }; },
        async embed() { return { embedding: [0] }; },
    };
}
...
function createSlowProvider(delayMs) {
    return {
        async chat() {
            ...
            return { role: 'assistant', content: 'slow', usage: { promptTokens: 5, completionTokens: 5 } };
        },
        async *stream() { yield { delta: 'slow' }; },
        async embed() { return { embedding: [0] }; },
    };
}
...
describe('MiddlewarePipeline', () => {
    it('should execute middleware in order', async () => {
        ...
        pipeline.use('first', async (ctx, next) => { order.push(1); await next(); order.push(4); });
        pipeline.use('second', async (ctx, next) => { order.push(2); await next(); order.push(3); });
...
describe('loggingMiddleware', () => {
    it('should record latency and success', async () => {
        ...
        pipeline.use(async (ctx, next) => { ctx.result = { text: 'ok' }; });
...
describe('cacheMiddleware', () => {
    it('should cache and return cached results', async () => {
        ...
        pipeline.use(async (ctx, next) => { callCount++; ctx.result = { text: 'fresh' }; });
...
describe('rateLimitMiddleware', () => {
    it('should throw when rate limit exceeded', async () => {
        ...
        pipeline.use(async (ctx, next) => { ctx.result = 'ok'; });
