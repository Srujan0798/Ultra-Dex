...
// ──────────────────────────────────────────────────────────────────────────────
// Platform logger (optional dep — graceful fallback if not available)
// ──────────────────────────────────────────────────────────────────────────────
...
async function loadPlatformLogger(): Promise<void> {
  try {
    const mod = await import('../src/core/monitoring/better-stack-logger.js');
    _platformLogger = mod.logger;
  } catch {
    // Platform logger not available (tests, standalone use) — use stderr
  }
}

// Best-effort load; DexGraph works without it
loadPlatformLogger().catch(() => undefined);
...
class DexLoggerImpl implements DexLogger {
  ...
  info(event: string, meta?: Record<string, unknown>): void {
    ...
    if (_platformLogger) {
      _platformLogger.info(`[dexgraph] ${event}`, m);
  ...
  warn(event: string, meta?: Record<string, unknown>): void {
    const m = this.buildMeta(meta);
    if (_platformLogger) {
      _platformLogger.warn(`[dexgraph] ${event}`, m);
    } else {
  ...
  error(event: string, err: unknown, meta?: Record<string, unknown>): void {
    ...
    if (_platformLogger) {
      _platformLogger.error(`[dexgraph] ${event}`, m);
    } else {
