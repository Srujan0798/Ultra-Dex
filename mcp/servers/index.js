...
export class BaseMCPServer extends EventEmitter {
  ...
  getStats() {
    return {
      ...
      ...this.stats,
      avgLatencyMs:
        this.stats.calls > 0 ? Math.round(this.stats.totalLatencyMs / this.stats.calls) : 0,
    };
  }
...
export class MemoryMCPServer extends BaseMCPServer {
  constructor({ memoryApi = null } = {}) {
    ...
    this.registerTool('store', {
      ...
      inputSchema: {
        type: 'object',
        properties: { context: { type: 'object' }, options: { type: 'object' } },
      },
    ...
    this.registerTool('query_graph', {
      ...
      inputSchema: {
        type: 'object',
        properties: { entity: { type: 'string' }, depth: { type: 'number' } },
      },
    ...
    this.registerTool('execute', {
      ...
      handler: async ({ language, code }) => {
        if (!this.allowedLanguages.includes(language)) {
          throw new Error(
            `Language "${language}" not allowed. Use: ${this.allowedLanguages.join(', ')}`
          );
        }
    ...
    this.registerTool('eval_expression', {
      ...
      handler: async ({ expression }) => {
        try {
          ...
          const result = vm.runInNewContext(`'use strict'; (${expression})`, context, {
            timeout: 1000,
          });
  ...
  _execute(language, code) {
    ...
    try {
      ...
      if (result.status !== 0) {
        return {
          language,
          output: result.stderr || result.stdout || 'Error',
...
export class MCPServerRegistry {
  ...
  discoverByCapability(keyword) {
    return this.listAllTools().filter(
      (t) => t.name.includes(keyword) || t.description.toLowerCase().includes(keyword.toLowerCase())
    );
  }
